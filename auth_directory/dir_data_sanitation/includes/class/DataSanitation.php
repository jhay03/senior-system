...
class DataSanitation extends DataSanitationRules
{
  ...
  public function getAllMD($conn_pdo, $get_name, $get_class)
  {
    try {
        $array_result = [];
        $name ="%".$get_name."%";
        if($get_class == 'ALL'){
          $sql = "SELECT DISTINCT(REPLACE(sanit_mdname, ',', '')) as NAME, sanit_mdname AS ORIG_NAME FROM db_sanitation
                  WHERE REPLACE(sanit_mdname, ',', '') LIKE :sanit_mdname ORDER BY sanit_mdname ASC LIMIT 50";
        }else {
          $sql = "SELECT DISTINCT(REPLACE(sanit_mdname, ',', '')) as NAME, sanit_mdname AS ORIG_NAME
                  FROM db_sanitation
                  WHERE REPLACE(sanit_mdname, ',', '') LIKE :sanit_mdname
                  AND sanit_group = :sanit_group
                  ORDER BY sanit_mdname ASC LIMIT 500";
        }

        $stmt = $conn_pdo->prepare($sql);
        $stmt->bindParam(':sanit_mdname', $name, PDO::PARAM_STR);
        if($get_class != 'ALL'){
          $stmt->bindParam(':sanit_group', $get_class, PDO::PARAM_STR);
        }
        $stmt->execute();
        $data = $stmt->fetchAll();
  ...
  public function getDataForNonMasterlist($conn_pdo, $district)
  {
    try {
        ini_set('memory_limit', '2048M');
        $placeholders = str_repeat('?,', count($district)-1).'?';
        $sql = "SELECT raw_id, raw_doctor, raw_license, raw_address, raw_branchname, raw_lbucode, raw_hospcode, raw_sarcode, raw_amount,
                       raw_corrected_name, sanitized_by, date_sanitized
                FROM sanitation_result1
                WHERE raw_district IN($placeholders)"; // LIMIT 1000
        $stmt = $conn_pdo->prepare($sql);
        $stmt->execute($district);
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function getDataForNonMasterlistByLimit($conn_pdo, $district, $from)
  {
    try {
        ini_set('memory_limit', '2048M');
        $placeholders = str_repeat('?,', count($district)-1).'?';
        $sql = "SELECT raw_id, raw_doctor, raw_license, raw_address, raw_branchname, raw_lbucode, raw_hospcode, raw_sarcode, raw_amount,
                       raw_corrected_name, sanitized_by, date_sanitized
                FROM sanitation_result1
                WHERE raw_district IN($placeholders) LIMIT $from, 1000";
        $stmt = $conn_pdo->prepare($sql);
        $stmt->execute($district);
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function updateDataSanitationResult1($conn_pdo, $raw_id, $md, $conn_pdo2, $authUser, $auth_usercode, $table_name, $raw_table_name)
  {
    try {
        $array_result = [];
        ...
        if($stmt->errorCode() == 0){
          ...
          $this->getDataForBroadCast($raw_id, $md, $authUser);
          //-----------END BROADCAST
          $response = [];
          $data_result = $this->getDataForRules($conn_pdo, $raw_id, $raw_table_name);
          foreach ($data_result as $key => $value) {

            foreach ($table_name as $key_2 => $value_2) {
            ...
            //----------------------add data rule application module here
            $this->executeBackgroundProcess($value[0], $md, $authUser);
            //-------------end
            $this->insertRulesAndLogs($conn_pdo2, '$category', $md, $auth_usercode, $authUser, count($table_name), $value[0]);
          }
  ...
  public function executeBackgroundProcess($rule_code, $md, $authUser)
  {
    $code = escapeshellarg($rule_code);
    $doctor = escapeshellarg($md);
    $sanitized_by = escapeshellarg($authUser);

    return $this->backgroundProcess("background-process.php {$code} {$doctor} {$sanitized_by}");
  }
  ...
  public function getDataForRules($conn_pdo, $raw_id, $raw_table_name)
  {
    try {
      $placeholders = str_repeat('?,', count($raw_id)-1).'?';
      $sql = "SELECT $raw_table_name FROM sanitation_result1 WHERE raw_id IN ($placeholders)";
      $stmt = $conn_pdo->prepare($sql);
      $stmt->execute($raw_id);
      $data = $stmt->fetchAll();
      if($data){
        $counter = 0;
        $table_name = explode(',', $raw_table_name);
        foreach ($data as $row) {
    ...
    $pieces = array_chunk($array_result, ceil(count($array_result) / count($raw_id)));
    // $pieces = array_slice($array_result, count($table_name) - 1);

    return $this->removeDuplicate($pieces);
    } catch (PDOException $e) {
        throw new Exception("Connection failed: ". $e->getMessage());
    }
  }
  ...
  public function removeDuplicate($array_list)
  {
    ...
    // $new_value[]  = implode("||", $array_list);
    $new_value = array_unique($new_value);
    foreach ($new_value as $value) {
      $new = explode("||", $value);
      $rule_code = array($this->generateRuleCode());
      $sanitized_result[] = array_merge($rule_code,$new);
        ...
        // $branch = $new[3] ;
        // $lbu = $new[4] ;
        // $sanitized_result[] = array($rule_code, $dr, $license, $address, $branch, $lbu);
    }
  ...
  public function setUnclasified($conn_pdo, $raw_id, $md, $authUser)
  {
    try {
        $array_result = [];
        $placeholders = str_repeat('?,', count($raw_id)-1).'?';
        ...
        $sql = "UPDATE sanitation_result1
                ...
                WHERE raw_id IN ($placeholders)";
        $stmt = $conn_pdo->prepare($sql);
        $params = array_merge([$status], $raw_id, [$authUser]);
        $stmt->execute($params);
        if($stmt->errorCode() == 0){
          //BROADCAST UPDATED DATA
          $this->getDataForBroadCast($raw_id, $md, $authUser);
  ...
  public function addMD($conn_pdo, $sanit_mdname, $sanit_mdcode, $sanit_group, $sanit_universe)
  {
    $name_basis = ', ,';
    try {
      $array_result = [];
      $sql = "INSERT INTO db_sanitation ( sanit_mdname, sanit_mdcode, sanit_group, sanit_universe, sanit_namebasis )
                              VALUES ( :sanit_mdname, :sanit_mdcode, :sanit_group, :sanit_universe, :sanit_namebasis )";
      $stmt = $conn_pdo->prepare($sql);
      $stmt->bindParam(':sanit_mdname',  $sanit_mdname, PDO::PARAM_STR);
      $stmt->bindParam(':sanit_mdcode',  $sanit_mdcode, PDO::PARAM_STR);
      $stmt->bindParam(':sanit_group',  $sanit_group, PDO::PARAM_STR);
      $stmt->bindParam(':sanit_universe',  $sanit_universe, PDO::PARAM_STR);
      $stmt->bindParam(':sanit_namebasis',  $name_basis, PDO::PARAM_STR);
      $stmt->execute();
      return 'success';
    } catch (PDOException $e) {
      throw new Exception("Connection failed: ". $e->getMessage());
  ...
  public function getRulesDetails($conn_pdo, $rule_code)
  {
    try {
        $array_result = '';
        $sql = "SELECT details_column_name, details_value_optr, details_value, details_condition_optr
                FROM rules_details
        ...
        if($data){
          foreach ($data as $row) {
              $array_result .= $row['details_column_name'] . ' ' . $row['details_value_optr'] . " '" . $row['details_value'] . "' " . $row['details_condition_optr'] . ' ';
            }
        }else {
          $array_result = 0;
        }
  ...
  public function countRuleDetailsMatch($conn_pdo, $where)
  {
    try {
        $array_result = 0;
        $sql = "SELECT count(raw_id) as TOTAL, raw_id as xxx
                FROM sanitation_result1
                WHERE $where AND sanitized_by = ''";
        $stmt = $conn_pdo->prepare($sql);
        $stmt->bindParam(':rule_code', $rule_code, PDO::PARAM_STR);
        $stmt->execute();
        $data = $stmt->fetchAll();
  ...
  public function getRawID($conn_pdo, $where)
  {
    try {
        $array_result = [];
        $sql = "SELECT raw_id
  ...
  public function backgroundProcess($cmd)
  {
    if (substr(php_uname(), 0, 7) == "Windows"){
        ...
        pclose(popen("start /B C:/xampp/php/php.exe ". $cmd, "r"));
    }else{
        exec($cmd . " > /dev/null &");
    }
  ...
  public function getDataForBroadCast($raw_id, $md, $authUser)
  {
    ...
    $data[] = array(
        "raw_id" => $id,
        "raw_corrected_name" => $md,
        "sanitized_by" => $authUser,
        "sanitized_date" => $current_date
      );
    }
    return $pusher->trigger('data-sanit-101', 'client-my-event', $data);
  }

  public function applyRules($conn_pdo, $raw_status, $raw_mdcode, $raw_corrected_name, $raw_universe, $sanitized_by, $raw_id)
  {
    ...
    try {
        ...
        $sql = "UPDATE sanitation_result1
                ...
                WHERE raw_id = :raw_id";
        $stmt = $conn_pdo->prepare($sql);
        $stmt->bindParam(':raw_status', $raw_status, PDO::PARAM_STR);
        $stmt->bindParam(':raw_mdcode', $raw_mdcode, PDO::PARAM_STR);
        $stmt->bindParam(':raw_corrected_name', $raw_corrected_name, PDO::PARAM_STR);
        $stmt->bindParam(':raw_universe', $raw_universe, PDO::PARAM_STR);
        $stmt->bindParam(':sanitized_by', $sanitized_by, PDO::PARAM_STR);
  ...
  public function resetUnsanitizedData($conn_pdo)
  {
