<?php
  ...
  // Function Get All in one query 08/212/2018
  function getAllRecords($mdname,$prodcode,$lbacode,$year,$month){
    ...
    $amountQuery = $mysqli -> query("SELECT 'mdprodname' as branchname,SUM(data_amount) as total FROM mdc_senior.senior_data as a INNER JOIN mdc_senior.mst_database as b ON a.data_branchcode=b.mst_branchcode
                                     WHERE data_doctor='$mdname' AND mst_lburebate='$lbacode' AND data_year='$year' AND data_month='$month' AND data_prodcode='$prodcode' GROUP BY mst_lburebate
                                     UNION ALL
                                     ...
                                     WHERE mst_lburebate='$lbacode' AND data_year='$year' AND data_month='$month' AND data_prodcode='$prodcode' GROUP BY mst_lburebate
                                     UNION ALL
                                     ...
                                     WHERE data_doctor='$mdname' AND mst_lburebate='$lbacode' AND data_year='$year' AND data_month='$month' AND data_prodcode='$prodcode' GROUP BY data_branchcode
                                     UNION ALL
                                     ...
                                     WHERE mst_lburebate='$lbacode' AND data_year='$year' AND data_month='$month' AND data_prodcode='$prodcode'
...
<table id="_example1" class="table table-bordered table-striped" style="margin-top:0 !important;" width="100%">
  <thead style="background:#d7ebf9 url(../../dist/img/bg.png) 50% 50% repeat-x;border: 1px solid #FFF;padding:12px;color:#2779aa">
      <tr style="background:#d7ebf9 url(../../dist/img/bg.png) 50% 50% repeat-x;border: 1px solid #FFF;padding:12px;color:#2779aa">
          ...
          <?php
            if($tyear == $fyear){
              for($i = $fmonth; $i <= $tmonth; $i++){
                ...
                ${'allRecords'.$i.$tyear} = getAllRecords($mdname,$prodcode,$lbacode,$tyear,date('F', mktime(0,0,0,$i,10)));
          ...
          <?php
            if($fyear < $tyear && ($tyear-$fyear)>=2){
              for($m=$month_start; $m<=12; $m++){
                ...
                // Implementation 08/28/2018
                ${'allRecords'.$m.$fyear} = getAllRecords($mdname,$prodcode,$lbacode,$fyear,date('F', mktime(0,0,0,$m,10)));
            ...
            <?php
              ...
              for($year=$mid_s;$year<=$mid_e;$year++){
                for($m=1; $m<=12; $m++){
                  ...
                  // Implementation 08/28/2018
                  ${'allRecords'.$m.$year} = getAllRecords($mdname,$prodcode,$lbacode,$year,date('F', mktime(0,0,0,$m,10)));
            ...
            <?php
              ...
              $month_ends = date('F', mktime(0,0,0,$m,10));
              // Implementation 08/28/2018
              ${'allRecords'.$m.$tyear} = getAllRecords($mdname,$prodcode,$lbacode,$tyear,date('F', mktime(0,0,0,$m,10)));
