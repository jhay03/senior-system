...
<div class="modal fade in" id="modalSettings" style="display: block; padding-right: 17px;">
...
<div id="modalConfirmation" class="modal fade" role="dialog">
        ...
        <div class="col-lg-12">
          <div class="box box-success">
            ...
            <div class="box-body">
              ...
              <table id="example2" class="table table-bordered table-hover" style="font-size:12px;white-space: nowrap;" width="100%">
                <thead>
                  <tr>
                    <th></th>
                    <th>
        ...
        <div class="col-lg-12">
            <div class="box box-success">
              <div class="box-body">
                <div>
					             Show/Hide Column:
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="0">ID</button>
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="1">Doctor</button>
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="2">LN</button>
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="3">Location</button>
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="4">Branch Name</button>
                       <button class="toggle-vis btn btn-warning btn-xs" data-column="5">LBA</button>
				         </div>
                 <br>
                <table id="example5" style="width:100%" class="table table-bordered">
                  <thead>
                    <tr>
                      <th style="white-space: nowrap;">ID</th>
                      <th style="white-space: nowrap;">Doctor</th>
...
$(document).ready(function() {
  ...
  $('#example5 thead tr th').on("click", function(event) {
    if ($(event.target).is("button")) {
      table.columns().orderable(false).draw();
      // table.column(1).orderable(false).draw();
    }

  });

  // var table = $('#example5').DataTable({
  //   ajax: "includes/controller/get_data_sanitation.php?district=CENTRAL LUZON&mdname=GO",
  // });
setTimeout(function(){
   var table = $('#example5').DataTable({
     scrollY: "45vh",
     ...
     initComplete: function(settings, json) {
      this.api().columns().every(function() { //[1,2]
        var column = this;

        var select = $('<select class="form-control select2 selectpicker" multiple="multiple" data-width="fit" data-container="body" ><option value="" >Show All</option></select>')
        ...
        column.data().unique().sort().each(function(d, j) {
          var dataCount = column.rows(':contains('+d+')').data().length;
  ...
  function searchDataSanitation(district_list, doctor_list){
    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          // }else {
          //   alert("Doctor's name is not on the list!");
          // }
      }
    });
  ...
  function getDistrictDetails(district){
    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          $('#lblDistrictDetails').html(response[0].rowsCount + ' Unsanitized Records in ' + response[0].districtname);
          // $('#lblDistrictDetails').html(response[0].district_rowcount + ' Unsanitized Records in ' + response[0].district_name);
      }
    });
  }
