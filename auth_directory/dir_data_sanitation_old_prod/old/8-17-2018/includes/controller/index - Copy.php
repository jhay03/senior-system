...
$md_list = $dataSanitation->getFilteredGroupPerDistrict('raw_doctor', $pdoConnSanitationResultDB, '$data', 'CENTRAL LUZON', '$category', 'GO LUIS RAYMOND', $fuzz, '$filtered_md', '$license', '$loc', '$branch', '$lba');
$count = count($md_list);
if($count > 0){
...
if(isset($_POST['cmd'])){
  ...
  if ($cmd == 'get_filtered_group_data') {
    ...
    $lba = $_POST['lba'];
    $filtered_by = $_POST['filtered_by'];
    $md_list = $dataSanitation->getFilteredGroupPerDistrict($filtered_by, $pdoConnSanitationResultDB, $data, $district, $category, $md, $fuzz, $filtered_md, $license, $loc, $branch, $lba);
    $count = count($md_list);
    if($count > 0){
    ...
    $dataSanitation = new DataSanitation();
    $district = $_POST['district'];
    $result = $dataSanitation->getDistrictDetails($conn_pdo, $district, $pdoConnSanitationResultDB);
    ...
    // $raw_id = $_POST['raw_id'];
    $md = $_POST['doctor_list'];
    $result = $dataSanitation->checkMDname($conn_pdo, $md);
