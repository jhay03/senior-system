...
class DataSanitation extends DataSanitationRules
{
  ...
  public function countDistrictRow($conn_pdo, $district)
  {
    try {
        $array_result = [];
        $placeholders = str_repeat('?,', count($district)-1).'?';
        $sql = "SELECT COUNT(raw_id) as TOTAL FROM sanitation_result1 WHERE raw_district IN ($placeholders) AND raw_status = ''";
        $stmt = $conn_pdo->prepare($sql);
        $stmt->execute($district);
        $data = $stmt->fetchAll();
        if($data){
  ...
  public function getDataPerDistrict($conn_pdo, $district)
  {
    try {
        $placeholders = str_repeat('?,', count($district)-1).'?';
        $sql = "SELECT raw_id, raw_doctor, raw_license, raw_address, raw_branchname, raw_lbucode FROM sanitation_result1 WHERE raw_district IN($placeholders) AND raw_status = ''";
        $stmt = $conn_pdo->prepare($sql);
        $stmt->execute($district);
        $data = $stmt->fetchAll();
